...
public class JdkDynamicProxyTest {
    public static void main(String[] args) throws Exception {
        ...
        Person personProxy =
                (Person) Proxy.newProxyInstance(Person.class.getClassLoader(), new Class<?>[] {Person.class}, Handler);
        /** 代理类信息 */
        ...
        /**
         * 动态代理对象步骤 1、 创建一个与代理对象相关联的 InvocationHandler，以及真实的委托类实例
         * 2、Proxy类的getProxyClass静态方法生成一个动态代理类stuProxyClass，该类继承Proxy类，实现 Person.java接口；JDK动态代理的特点是代理类必须继承Proxy类 3、通过代理类
         * proxyClass 获得他的带InvocationHandler 接口的构造函数 ProxyConstructor 4、通过 构造函数实例 ProxyConstructor 实例化一个代理对象，并将
     ...
     */
    public static void saveClass(String className, Class<?>[] interfaces,
            String pathdir) { /**
                               * 第一个参数是 代理类 名 。 第二个参数是 代理类需要实现的接口
                               */
